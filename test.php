...
if (!empty($search)) {
    $query = "SELECT * FROM Items WHERE ITEM_STATUS = '1' AND (ITEM_TYPE LIKE :search OR ITEM_NAME LIKE :search)";
        ...
        <div class="showcase container-fluid">
            ...
            if (!empty($product_array)) {
                foreach ($product_array as $key => $value) {
                    displayProduct($product_array[$key]);
                }
            }
            ?>
        </div>
...
<?php
// Functions for action handling and product display
...
?>
    <div class="col-sm-3">
        <div class="images text-center mx-auto">
            <a id="images" href="item.detail.php?detailid=<?php echo $product["PRODUCT_NO"] ?>">
                <img src="<?php echo 'admin/' . $product["ITEM_IMAGE"]; ?>" width="70%" />
                <h5><?php echo $product["ITEM_NAME"]; ?></h5>
                <p><?php echo $product["BRAND"]; ?></p>
                <p>more details...</p>
                <p><?php echo "GH₵" . $product["PRICE"]; ?></p>
            </a>
            <form method="post" action="shop.php?action=add&code=<?php echo $product["PRODUCT_NO"] ?>">
